...
// Write modes
enum putimage_ops{ COPY_PUT, XOR_PUT, OR_PUT, AND_PUT, NOT_PUT, TRANSPARENT_PUT, TRANSPARENTBK_PUT };
...
void putimage( int left, int top, IMAGE *bitmap, int op , int width=0, int height=0);
void printimage(
...
void imageputpixel(IMAGE *,int,int,int);
IMAGE *imageturn(IMAGE *image, double a, int color);
int imagegetpixel(IMAGE *,int,int);
IMAGE *loadBMP(const char *);
void saveBMP(const char *, IMAGE *);
